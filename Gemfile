
# $JHm/Gemfile

# bundle ins  after changes here

source "https://rubygems.org"

# ------
# 0 Ruby
# ------
# https://pages.github.com/versions.json

# --------
# 1 Jekyll
# --------
# some of these were needed
# gem 'activesupport'
# gem 'faraday-retry'  # for  jekyll doctor
# gem 'html-pipeline'
# gem 'i18n'
# gem 'jekyll'
# gem 'jekyll', '~> 3.10.0'
# gem 'jekyll-commonmark'
# gem 'jemoji'
# gem 'kramdown', '~> 2.4.0'
# gem 'kramdown-parser-gfm'
# gem 'liquid', '~> 3.0.6'
# gem 'liquid'

# --------------
# 2 GitHub Pages
# --------------
gem "github-pages", group: :jekyll_plugins
# - bundle update github-pages
# - https://rubygems.org/gems/github-pages  for versions

# gem "webrick", "~> 1.7"  # instead of using Ruby v2.7.4,
#  and still required even though  GitHub Pages  now uses  jekyll 3.9.3...

# Lock `http_parser.rb` gem to `v0.6.x` on JRuby builds since newer versions
#  of the gem do not have a Java counterpart.
# gem "http_parser.rb", "~> 0.6.1", :platforms => [:jruby]  # JH upped the patch to 1

# Performance-booster for watching directories on Windows
# gem "wdm", "~> 0.1.1", :platforms => [:mingw, :x64_mingw, :mswin]  # JH 2026-05-18-Mon
# gem "wdm", "~> 0.1.1", :platforms => [:windows]

# ---------
# 3 Plugins
# ---------
group :jekyll_plugins do
  # gem "jekyll-feed", "~> 0.12"
  gem "jekyll-feed"
end

# -----------
# 4 Theme-ish
# -----------
gem "jekyll-include-cache", group: :jekyll_plugins  # required by  Minimal Mistakes
gem "jekyll-remote-theme"  # to be able to use the latest version of minima theme
gem 'jekyll-seo-tag'
gem "minima", "~> 2.5.1"  # (https://rubygems.org/gems/minima) default theme for new Jekyll sites

# ----------
# 5 zoneinfo
# ----------
# Windows and JRuby does not include zoneinfo files,
#  so bundle the tzinfo-data gem and associated library.
# platforms :mingw, :x64_mingw, :mswin, :jruby do  # JH 2026-05-18-Mon
platforms :windows, :jruby do
  gem "tzinfo", ">= 1", "< 3"
  gem "tzinfo-data"
end

